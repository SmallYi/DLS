...
class FAHP():
    ...
    def get_global_weight(self,total_matrix):
        m = total_matrix[0].shape[0] # 第一层判断矩阵的行数
        n = len(total_matrix)
        weight = []
        for i in range(0,n):
    ...
    def get_weight(self,judge_matrix): #输入判断矩阵，得到对应权重向量
        n = len(judge_matrix)
        a = []
        weigh = []
    ...
    def judge_consistency(self,matrix): #判断模糊矩阵的一致性
        n = len(matrix)
        for i in range(0,n):
            for j in range(0,n):
                for k in range(0,n):
                    if matrix[i,j] == round((matrix[i,k] - matrix[j,k] + 0.5),1): # 保留浮点数一位小数
                        pass
                    else:
                        return 0
        return 1

    def turn_consistency(self,martix): #一致性变换
        r = []
        n = len(martix)
        for i in range(0,n):
            temp = 0
    ...
    def Vector_normal(self,vec): #对向量归一化
        n = len(vec)
        s = sum(vec)
        for i in range(0,n):
            vec[i] /= s
...
class Evaluation():
    ...
    def Membership_function(self,judge_set,indicator): # 隶属函数
        n = len(judge_set)
    ...
    def Membership_matrix(self,matrix_level,vector_indicator): #隶属矩阵
...
dataframem = pd.DataFrame(matrix_level,columns=head1)
...
dataframem.to_csv('Matrix_level.csv',index=False, sep=',')
